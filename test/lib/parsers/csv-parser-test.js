...
describe('csv parse', () => {
    ...
    it('returns promise', () => {
        expect(parser('')).toBeInstanceOf(Promise)
    })

    it('parses pased file', () => {
        let path = `/path/to/file/${faker.random.uuid()}.txt`
        parser(path)
        expect(fs.createReadStream.mock.calls[0][0]).toBe(path)
    })

    it('parses parses data based on passed delimiter', () => {
        let result = parser('', ',')
        return expect(result).resolves.toEqual([["1234","2345"],["4444","5555"]])
    })
})